...
interface RebuildPayload {
  leadMagnetId: string;
  userId: string;
  /** Status to restore after generation completes (e.g. 'draft'). Defaults to 'draft'. */
...
export const rebuildLeadMagnetContent = task({
  ...
  run: async (payload: RebuildPayload) => {
    ...
    const restoreStatus = payload.previousStatus || 'draft';
    ...
    try {
      ...
      // Restore previous status (clear 'processing' flag)
      await supabase
        .from('lead_magnets')
      ...
      // Reset status so client stops polling and user can retry
      await supabase
        .from('lead_magnets')
        .update({ status: restoreStatus })
        .eq('id', leadMagnetId);

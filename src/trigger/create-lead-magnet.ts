...
export const createLeadMagnetPipeline = task({
  ...
  run: async (payload: CreateLeadMagnetPipelinePayload) => {
    const { userId, userName, username, archetype, businessContext, topic, leadMagnetId, teamId } =
      payload;
    ...
    try {
      ...
      const ideationResult = await generateLeadMagnetIdeasParallel(
        fullBusinessContext,
        undefined,
        userId
      );
      ...
      try {
        const { data: brandKit } = await supabase
          .from('brand_kits')
          .select(
            'business_description, sender_name, best_video_url, best_video_title, content_links, community_url'
          )
          .eq('user_id', userId)
          .single();
        ...
        const emailContext: EmailGenerationContext = {
          ...
          senderName,
          businessDescription:
            brandKit?.business_description || businessContext.businessDescription,
          bestVideoUrl: brandKit?.best_video_url || undefined,
          bestVideoTitle: brandKit?.best_video_title || undefined,
          contentLinks: brandKit?.content_links as
      ...
      // ==========================================
      // STEP K: Fire DFY callback (if engagement-triggered)
      // ==========================================
      if (payload.engagementId) {
        fireDfyCallback({
          engagement_id: payload.engagementId,
          automation_type: 'lead_magnet_generation',
          status: 'completed',
          result: {
            lead_magnet_id: leadMagnetId,
            title: selectedConcept.title,
            funnel_url: funnelResult?.url ?? null,
          },
        }).catch(() => {
      ...
      // Fire failure callback before re-throwing
      if (payload.engagementId) {
        fireDfyCallback({
          engagement_id: payload.engagementId,
          automation_type: 'lead_magnet_generation',

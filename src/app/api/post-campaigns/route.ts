...
export async function GET(request: NextRequest) {
  try {
    ...
    const status = request.nextUrl.searchParams.get('status') as PostCampaignStatus | null;
    const result = await service.listCampaigns(session.user.id, status ?? undefined);

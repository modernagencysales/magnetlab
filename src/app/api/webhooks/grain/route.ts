...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!secret || secret !== process.env.GRAIN_WEBHOOK_SECRET) {
      return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
    }

    const payload: GrainWebhookPayload = await request.json();

    if (!payload.recording_id || !payload.transcript || !payload.user_id) {
      return NextResponse.json(
        { error: 'Missing required fields: recording_id, transcript, user_id' },
        { status: 400 }
      );
    }

    const supabase = createSupabaseAdminClient();
    const externalId = `grain:${payload.recording_id}`;

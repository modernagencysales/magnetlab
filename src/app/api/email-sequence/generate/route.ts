...
export async function POST(request: Request) {
  try {
    ...
    // Get user name + username as fallback for sender name and content URL
    ...
    // Resolve resource URL: external_url > hosted content page > null
    ...
    if (externalUrl && /^https?:\/\/.+/.test(externalUrl)) {
      ...
      // Need the funnel slug to build the content URL
    ...
    if (resourceUrl) {
      ...
      if (email1 && !email1.body.includes(resourceUrl)) {
        // Inject resourceUrl before the sign-off line (last non-empty line)
    ...
    // Upsert email sequence (auto-activate when resource URL is available)

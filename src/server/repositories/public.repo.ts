...
/** Get lead magnet title, external_url, content for lead capture (resource URL + webhooks). */
export async function findLeadMagnetForLead(leadMagnetId: string) {
  const supabase = createSupabaseAdminClient();
  const { data } = await supabase
    .from('lead_magnets')
...
/** Find lead by id (for PATCH qualification). */
...
/** Get qualification_form_id for a funnel page. */
export async function findFunnelPageQualificationFormId(funnelPageId: string): Promise<string | null> {
  const supabase = createSupabaseAdminClient();
  const { data } = await supabase
    .from('funnel_pages')
    ...
    .eq('id', funnelPageId)
    .single();
  return data?.qualification_form_id ?? null;
}

/** Get funnel slug and lead_magnet title (for PATCH webhook payload). */
export async function findFunnelSlugAndLeadMagnetTitle(funnelPageId: string) {
  const supabase = createSupabaseAdminClient();
  const { data } = await supabase
    .from('funnel_pages')
    ...
    .single();
  if (!data) return null;
  const leadMagnets = data.lead_magnets as { title: string } | { title: string }[] | null;
  const title = Array.isArray(leadMagnets) ? leadMagnets[0]?.title ?? '' : leadMagnets?.title ?? '';
  return { slug: data.slug, leadMagnetTitle: title };
}

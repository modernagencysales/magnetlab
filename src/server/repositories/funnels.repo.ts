...
export async function updateFunnelPageByIdUnscoped(
  ...
  if (error) throw new Error(`funnels.updateFunnelPageByIdUnscoped: ${error.message}`);
}
...
export async function findFunnelByIdAndUserIdWithLeadMagnet(
  ...
  const { data, error } = await supabase
    ...
    .single();
  if (error || !data) return null;
  return data as FunnelPageRow & { lead_magnets?: { id: string } };
}

/** Get funnel page id by lead_magnet_id (single). */
export async function findFunnelPageIdByLeadMagnetId(leadMagnetId: string): Promise<string | null> {
  const supabase = createSupabaseAdminClient();
...
/** Count page_views for a funnel page. */
export async function getPageViewCountByFunnelPageId(funnelPageId: string): Promise<number> {
  const supabase = createSupabaseAdminClient();
...
/** Count funnel_leads by lead_magnet_id. */
export async function getFunnelLeadCountByLeadMagnetId(leadMagnetId: string): Promise<number> {
  const supabase = createSupabaseAdminClient();
  const { count } = await supabase
    .from('funnel_leads')
    .select('*', { count: 'exact', head: true })
...
export async function getLeadMagnetForPublish(userId: string, leadMagnetId: string) {
  ...
  const { data } = await supabase
    .from('lead_magnets')
    .select('id, extracted_content, polished_content, concept, external_url, content, archetype')
    .eq('id', leadMagnetId)
    .eq('user_id', userId)
    .single();
  return data;

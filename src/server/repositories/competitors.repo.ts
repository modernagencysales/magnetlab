...
const CP_MONITORED_COMPETITORS_COLUMNS =
  'id, user_id, linkedin_profile_url, heyreach_campaign_id, is_active, created_at, updated_at';
...
export async function findCompetitorsByUserId(userId: string): Promise<CompetitorRow[]> {
  const supabase = createSupabaseAdminClient();
  const { data, error } = await supabase
    .from('cp_monitored_competitors')
    .select(CP_MONITORED_COMPETITORS_COLUMNS)
    .eq('user_id', userId)
...
export async function countEngagementsForCompetitor(competitorId: string): Promise<number> {
  const supabase = createSupabaseAdminClient();
  const { count, error } = await supabase
...
export async function deleteCompetitor(userId: string, id: string): Promise<void> {
  const supabase = createSupabaseAdminClient();

...
export async function getKnowledgeDigest(userId: string, days: number, teamId?: string) {
  return getRecentKnowledgeDigest(userId, Math.min(days, 90), teamId);
}

export async function getTopics(userId: string, teamId?: string, limit = 50) {
  const effectiveUserId = await resolveEffectiveUserId(userId, teamId);
  return listKnowledgeTopics(effectiveUserId, { limit });
}

export async function getTopicBySlug(userId: string, slug: string, teamId?: string) {
...
export async function getKnowledgeGaps(userId: string, teamId?: string, limit = 20) {
...
export async function assessKnowledgeReadiness(
  ...
  if (!VALID_GOALS.includes(goal)) {
    throw Object.assign(new Error(`goal must be one of: ${VALID_GOALS.join(', ')}`), {
      statusCode: 400,
    });
  }
...
export async function deleteKnowledgeEntry(userId: string, id: string): Promise<void> {
  ...
  await Promise.all((current.tags || []).map((t) => knowledgeRepo.decrementTagCount(userId, t)));

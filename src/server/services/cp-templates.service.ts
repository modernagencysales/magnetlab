...
const SEED_TEMPLATES = [
  ...
  },
  {
    ...
    structure:
      '[SET UP THE CONTEXT - 2-3 sentences]\n\n[THE QUESTION]\n\n[YOUR TAKE - brief]\n\n[INVITE RESPONSES]',
...
export async function create(
  ...
  try {
    const embeddingText = createTemplateEmbeddingText({
      ...
      use_cases: payload.use_cases as string[] | null | undefined,
...
export async function bulkImport(
  ...
  if (templates.length > 50)
    return {
      ...
      message: 'Maximum 50 templates per import',
    };
  ...
  for (const template of templates) {
    ...
    try {
      const embeddingText = createTemplateEmbeddingText({
        ...template,
        use_cases: template.use_cases as string[] | null | undefined,
        tags: template.tags as string[] | null | undefined,
      });
      embedding = await generateEmbedding(embeddingText);
    } catch {
      // continue without embedding

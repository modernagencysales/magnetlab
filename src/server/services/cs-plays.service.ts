...
function computePromotionSuggestion(
  ...
  const {
    promoteMinMultiplier,
    promoteMaxVariation,
    declineMaxMultiplier,
    minResultsForPromotion,
  } = PLAY_PROMOTION_THRESHOLDS;

  if (multipliers.length < minResultsForPromotion) return null;

  // Promote: testing status + avg >= threshold + low variance
  if (
    play.status === 'testing' &&
    avgMultiplier !== null &&
    avgMultiplier >= promoteMinMultiplier
  ) {
    ...
    if (coeffOfVariation < promoteMaxVariation) return 'promote';
  }

  // Decline: proven status + recent results avg below threshold
  if (play.status === 'proven' && multipliers.length >= minResultsForPromotion) {
    const recent = multipliers.slice(0, minResultsForPromotion);
    const recentAvg = recent.reduce((a, b) => a + b, 0) / recent.length;
    if (recentAvg < declineMaxMultiplier) return 'decline';
  }
...
export async function updateDataSharing(userId: string, enabled: boolean): Promise<boolean> {
  ...
  const { error } = await supabase
    ...
    .eq('id', userId);
  if (error) {
    throw Object.assign(new Error(`Failed to update data sharing: ${error.message}`), {
      statusCode: 500,
    });
  }
  return enabled;

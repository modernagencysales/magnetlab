/**
 * Knowledge Actions.
 * Copilot actions for searching the AI Brain, listing topics, and building content briefs.
...
registerAction({
  ...
  description:
    'Search the AI Brain knowledge base using semantic search. Always call this before writing content to ground it in real expertise from transcripts and calls.',
  parameters: {
    properties: {
  ...
  handler: async (
    ...
    const results = await searchKnowledgeV2(ctx.scope.userId, {
      query: params.query,
      knowledgeType: params.knowledge_type as
        | import('@/lib/types/content-pipeline').KnowledgeType
        | undefined,
...
registerAction({
  ...
  handler: async (ctx: ActionContext, params: { limit?: number }): Promise<ActionResult> => {
    const topics = await listKnowledgeTopics(ctx.scope.userId, {
      teamId: ctx.scope.teamId,
...
registerAction({
  ...
  description:
    'Build a knowledge-powered content brief for a topic. Returns relevant knowledge entries organized by type, suggested angles, and topic readiness score. Use this before writing to inject real expertise.',
  parameters: {
    properties: {
  ...
  handler: async (ctx: ActionContext, params: { topic: string }): Promise<ActionResult> => {
    const brief = await buildContentBrief(ctx.scope.userId, params.topic, {
      teamId: ctx.scope.teamId,
    });

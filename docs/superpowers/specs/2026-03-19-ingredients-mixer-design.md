...
| `/lead-magnets` | Lead magnet + pages combined | Old `/magnets` + `/pages` |
| *(removed)* `/knowledge` | Redirect to `/posts` | — |
| *(removed)* `/inspo` | Redirect to `/posts` | — |
| *(removed)* `/pages` | Redirect to `/lead-magnets` | — |
...
The only new prompt work. Builds context for the selected ingredients:
...
(delegated to existing buildVoicePromptSection())
...
| `magnetlab_mix` | `team_profile_id`, `exploit_id?`, `knowledge_topic?`, `knowledge_query?`, `style_id?`, `template_id?`, `creative_id?`, `trend_topic?`, `recycled_post_id?`, `instructions?`, `count?` (default 3), `output?` ("drafts" | "ideas", default "drafts") | Array of draft posts or idea cards. |
...
CREATE TABLE cp_mix_recipes (
  ...
  exploit_id UUID REFERENCES cp_exploits(id),
  knowledge_topic TEXT,
  knowledge_query TEXT,
  style_id UUID REFERENCES cp_writing_styles(id),
  template_id UUID REFERENCES cp_post_templates(id),
  creative_id UUID REFERENCES cp_creatives(id),
  trend_topic TEXT,
  recycled_post_id UUID REFERENCES cp_pipeline_posts(id),
  instructions TEXT,
  output_type TEXT NOT NULL DEFAULT 'drafts', -- 'drafts' | 'ideas'
...
Every mixer generation (UI or MCP) inserts a row. When a draft is sent to the pipeline, `post_ids` is updated.
...
- **Rollback:** Feature-flagged. Old pages can be restored by reverting sidebar config and removing redirects.

...
export class MagnetLabClient {
  ...
  async generateEmailSequence(params: { leadMagnetId: string; useAI?: boolean }) {
    return this.aiRequest<{ emailSequence: unknown; generated: boolean }>(
      'POST',
      `/email-sequence/generate`,
      params
    );
  }
  ...
  async updateEmailSequence(
    ...
    return this.request<{ emailSequence: unknown }>(
      'PUT',
      `/email-sequence/${leadMagnetId}`,
      params
    );
  ...
  async generateFlowEmails(flowId: string, stepCount?: number) {
    return this.aiRequest<unknown>(
      'POST',
      `/email/flows/${flowId}/generate`,
      stepCount ? { stepCount } : {}
    );
  }
  ...
  async sendBroadcast(id: string) {
    return this.request<{ message: string; recipientCount: number }>(
      'POST',
  ...
  async polishPost(id: string) {
    return this.aiRequest<{ success: boolean; polishResult: unknown }>(
      'POST',
      `/content-pipeline/posts/${id}/polish`,
      {}
    );
  }
  ...
  async getPostsByDateRange(params: { startDate: string; endDate: string }) {
    ...
    return this.request<{ posts: unknown[] }>(
      'GET',
      `/content-pipeline/posts/by-date-range?${searchParams}`
    );
  }
  ...
  async triggerAutopilot(params?: {
    ...
    return this.request<{ triggered: boolean; runId: string }>(
      'POST',
      `/content-pipeline/schedule/autopilot`,
      params || {}
    );
  ...
  async matchTemplate(params: { ideaId: string }) {
    ...
    const ideaResult = await this.request<{ idea: Record<string, unknown> }>(
      'GET',
      `/content-pipeline/ideas/${params.ideaId}`
    );
    ...
    if (!topic) {
      throw new Error(
        'Could not extract topic text from idea — idea may not exist or has no title/insight'
      );
    }
    return this.request<{ matches: unknown[] }>('POST', `/content-pipeline/templates/match`, {
      topic,
    });
  }
  ...
  async browseSwipeFilePosts(params?: {
    ...
    if (params?.limit) searchParams.set('limit', String(params.limit));
    if (params?.offset) searchParams.set('offset', String(params.offset));
    return this.request<{ posts: unknown[]; total: number }>(
      'GET',
      `/swipe-file/posts?${searchParams}`
    );
  }
  ...
  async browseSwipeFileLeadMagnets(params?: {
    ...
    return this.request<{ leadMagnets: unknown[]; total: number }>(
      'GET',
      `/swipe-file/lead-magnets?${searchParams}`
    );
  }
  ...
  async listQuestions(formId: string) {
    return this.request<{ questions: unknown[] }>(
      'GET',
  ...
  async createQuestion(formId: string, params: Record<string, unknown>) {
    return this.request<{ question: unknown }>(
      'POST',
      `/qualification-forms/${formId}/questions`,
      params
    );

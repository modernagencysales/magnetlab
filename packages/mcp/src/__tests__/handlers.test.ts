  ...
  describe('Email Sequences — camelCase transforms', () => {
    ...
    it('magnetlab_update_email_sequence transforms reply_trigger', async () => {
      await callTool('magnetlab_update_email_sequence', {
        lead_magnet_id: 'lm-1',
        emails: [{ day: 1, subject: 'Hi', body: 'Hello', reply_trigger: 'welcome' }],
        status: 'active',
      });
      expect(client.updateEmailSequence).toHaveBeenCalledWith('lm-1', {
        emails: [{ day: 1, subject: 'Hi', body: 'Hello', replyTrigger: 'welcome' }],
        status: 'active',
      });
    });

    it('magnetlab_activate_email_sequence passes lead_magnet_id', async () => {
      // activate now fetches the sequence first to validate placeholders
      (client.getEmailSequence as ReturnType<typeof vi.fn>).mockResolvedValue({
        emailSequence: {
          id: 'seq-1',
          status: 'draft',
          emails: [{ day: 0, subject: 'Welcome', body: 'Real content here.' }],
        },
      });

  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    if (name === 'magnetlab_tool_help') {
      ...
      if (!toolName) {
        return {
          content: [
            { type: 'text', text: JSON.stringify({ error: 'Missing required parameter: tool' }) },
          ],
        };
      }
          ...
          content: [
            {
              type: 'text',
              text: JSON.stringify({
                error: `Unknown tool: ${toolName}. Call a category tool first to see available tools.`,
              }),
            },
          ],
        };
    ...
    if (name === 'magnetlab_execute') {
      const toolName = (args as Record<string, unknown>)?.tool as string;
      const toolArgs =
        ((args as Record<string, unknown>)?.arguments as Record<string, unknown>) || {};

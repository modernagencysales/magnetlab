-- Seed all AI prompt templates from prompt-defaults.ts
-- All entries start with is_active = false (activated one-by-one later)
...
- 1-2 sentences max$usr$,
  ...
  RETURNING id, system_prompt, user_prompt, model, temperature, max_tokens
)
...
WITH inserted AS (
  ...
  VALUES (
    ...
    '',
    $usr$You are writing a LinkedIn post. Write the post without any preamble. Your first word is the first word of the post.
...
Now write the post. Return ONLY valid JSON:
{
  ...
  "cta_word": "simple keyword like interested, send, link"
    ...
    1.0,
    4000,
    '[{"name":"style_guidelines","description":"Base style guidelines from the style-guidelines prompt","example":"Voice:\\n- Direct, conversational..."},{"name":"voice_section","description":"Author voice section with first-person context, built by buildVoiceSection()","example":"YOU ARE WRITING AS: Tim Smith, CEO\\nFIRST-PERSON CONTEXT: I run a B2B agency..."},{"name":"voice_style_section","description":"Learned writing style patterns from buildVoicePromptSection()","example":"## Writing Style (learned from author edits)\\nTone: direct and conversational"},{"name":"target_audience","description":"Target audience for the post","example":"B2B professionals, agency owners, and marketers"},{"name":"idea_title","description":"Title of the content idea","example":"Why Most Lead Magnets Fail"},{"name":"idea_core_insight","description":"Core insight of the content idea","example":"Lead magnets fail because they deliver the outcome instead of creating an exchange mechanism."},{"name":"idea_full_context","description":"Full context for the content idea","example":"During our coaching call, we discussed how..."},{"name":"idea_why_post_worthy","description":"Why this idea is worth posting about","example":"Contrarian take that challenges conventional wisdom about lead magnets."},{"name":"idea_content_type","description":"Type of content (story, framework, contrarian, etc.)","example":"contrarian"},{"name":"knowledge_section","description":"Optional AI Brain knowledge context from calls (injected when available)","example":"KNOWLEDGE BASE CONTEXT (from your calls):\\n- Client saw 3x conversion after switching..."}]'::jsonb,
    false
  )
  RETURNING id, system_prompt, user_prompt, model, temperature, max_tokens
)
...
WITH inserted AS (
  ...
  VALUES (
    ...
    '',
    $usr$You are creating a LinkedIn post by combining a template with user-provided information.
...
Return ONLY valid JSON:
{
  ...
  "cta_word": "simple keyword like interested, send, link"
    ...
    1.0,
    4000,
    '[{"name":"template_structure","description":"The post template structure to follow","example":"Hook: [Bold claim with number]\\n\\nSetup: [1-2 sentences of context]\\n\\nSteps:\\n1. ..."},{"name":"template_examples","description":"Example posts using this template (optional, up to 2 examples)","example":"EXAMPLE POSTS USING THIS TEMPLATE:\\nI spent $50k on ads before I realized..."},{"name":"voice_section","description":"Author voice section with first-person context, built by buildVoiceSection()","example":"YOU ARE WRITING AS: Tim Smith, CEO\\nFIRST-PERSON CONTEXT: I run a B2B agency..."},{"name":"voice_style_section","description":"Learned writing style patterns from buildVoicePromptSection()","example":"## Writing Style (learned from author edits)\\nTone: direct and conversational"},{"name":"idea_title","description":"Title of the content idea","example":"Why Most Lead Magnets Fail"},{"name":"idea_core_insight","description":"Core insight of the content idea","example":"Lead magnets fail because they deliver the outcome instead of creating an exchange mechanism."},{"name":"idea_full_context","description":"Full context for the content idea","example":"During our coaching call, we discussed how..."},{"name":"idea_why_post_worthy","description":"Why this idea is worth posting about","example":"Contrarian take that challenges conventional wisdom about lead magnets."},{"name":"idea_content_type","description":"Type of content (story, framework, contrarian, etc.)","example":"contrarian"},{"name":"knowledge_section","description":"Optional AI Brain knowledge context from calls (injected when available)","example":"KNOWLEDGE BASE CONTEXT (from your calls):\\n- Client saw 3x conversion after switching..."},{"name":"target_audience","description":"Target audience for the post","example":"B2B professionals, agency owners, and marketers"}]'::jsonb,
    false
  )
  RETURNING id, system_prompt, user_prompt, model, temperature, max_tokens
)
...
Return ONLY the complete rewritten post (not just the changed section).$usr$,
    'claude-sonnet-4-6',
    1.0,
    2000,
    '[{"name":"section_instruction","description":"Instruction for which section to rewrite. One of: \"Rewrite just the opening/hook (first 1-2 sentences) to be more attention-grabbing.\", \"Rewrite the main body while keeping the same hook and CTA.\", \"Rewrite just the call-to-action at the end to be more engaging.\"","example":"Rewrite just the opening/hook (first 1-2 sentences) to be more attention-grabbing."},{"name":"post_content","description":"The current complete post text","example":"I spent $50k on LinkedIn ads last year.\\n\\nHere is what I learned..."},{"name":"feedback_section","description":"Optional feedback from the user about the rewrite","example":"FEEDBACK: Make the hook more specific with a dollar amount"},{"name":"style_guidelines","description":"Base style guidelines from the style-guidelines prompt","example":"Voice:\\n- Direct, conversational..."}]'::jsonb,
    false
  )
  RETURNING id, system_prompt, user_prompt, model, temperature, max_tokens
)
...
-- 5. banned-ai-phrases (config — JSON array stored in user_prompt)
WITH inserted AS (
  INSERT INTO ai_prompt_templates (slug, name, category, description, system_prompt, user_prompt, model, temperature, max_tokens, variables, is_active)
  VALUES (
    'banned-ai-phrases',
...
-- 6. hook-scoring-config (config — JSON object stored in user_prompt)
WITH inserted AS (
  INSERT INTO ai_prompt_templates (slug, name, category, description, system_prompt, user_prompt, model, temperature, max_tokens, variables, is_active)
  VALUES (
    'hook-scoring-config',
    'Hook Scoring Configuration',
    'scoring',
  ...
  "strength_factors": {
    ...
    "hasCuriosity": {
      "weight": 1.5,
      "pattern": "\\b(secret|surprising|unexpected|weird|strange|crazy|one thing|single)\\b",
      "description": "Creates curiosity gap"
    },
    "hasOutcome": {
      "weight": 2,
      "pattern": "\\b(revenue|profit|sales|customers|followers|growth|doubled|tripled|increased)\\b",
  ...
  "weakness_factors": {
    "isGeneric": {
      "penalty": 2,
      "pattern": "^(Tips for|Thoughts on|Some thoughts|How to|Ways to|Things to|Ideas for)\\b",
      "suggestion": "Start with a specific result or story instead of generic intro"
    },
    "isVague": {
      "penalty": 1.5,
      "pattern": "\\b(better|improve|great|good|important|essential|key|must)\\b",
    ...
    "lacksSpecificity": {
      ...
      "suggestion": "Add personal experience (I, my, we) or specific data"
    }
  }
}$usr$,
    'claude-sonnet-4-6',
...
-- 7. post-polish-rewrite
WITH inserted AS (
  INSERT INTO ai_prompt_templates (slug, name, category, description, system_prompt, user_prompt, model, temperature, max_tokens, variables, is_active)
  VALUES (
    'post-polish-rewrite',
    'Post Polish Rewriter',
    'content_writing',
    'Rewrites a LinkedIn post to fix AI-sounding phrases and strengthen weak hooks. Used by polishPost() via buildPolishPrompt().',
    '',
    $usr$You are an expert LinkedIn content editor. Rewrite the following post to fix these issues:
...
Return ONLY the rewritten post, no explanations or comments.$usr$,
    'claude-sonnet-4-6',
    1.0,
    2000,
    '[{"name":"issues_list","description":"Newline-separated list of issues found (AI phrases, weak hook score, etc.)","example":"AI-sounding phrases found: game-changer, deep dive\\nWeak hook (score: 4/10). Suggestions: Add specific numbers; Make it personal"},{"name":"voice_style_section","description":"Optional voice style section for author-specific polishing from buildVoicePromptSection()","example":"\\n## Writing Style (learned from author edits)\\nTone: direct and conversational\\n\\nPolish the post to match this author''s writing style.\\n"},{"name":"post_content","description":"The original post content to be polished","example":"Here''s the thing about lead magnets.\\n\\nThey''re a game-changer for your business..."}]'::jsonb,
    false
  )
  RETURNING id, system_prompt, user_prompt, model, temperature, max_tokens
)
...
WITH inserted AS (
  ...
  VALUES (
    ...
    '',
    $usr$Write a daily newsletter email for a B2B audience.
...
Return ONLY valid JSON with "subject" (compelling, 5-10 words) and "body" (markdown formatted).$usr$,
    'claude-sonnet-4-6',
    1.0,
    2000,
    '[{"name":"topic","description":"The topic for the newsletter email","example":"How to write cold emails that get replies"},{"name":"linkedin_topic_section","description":"Optional reference to today''s LinkedIn post topic for thematic consistency","example":"Today''s LinkedIn post topic (for thematic consistency, but write DIFFERENT content): Why most cold emails fail"},{"name":"author_section","description":"Optional author name","example":"AUTHOR: Tim Smith"},{"name":"knowledge_context","description":"Compiled knowledge base context for the topic","example":"KEY INSIGHTS:\\n- Cold emails with 3 sentences get 2x reply rate...\\nSTORIES:\\n- Client switched from 500 word emails to 3 sentences..."},{"name":"voice_style_section","description":"Learned writing style patterns from buildVoicePromptSection() for email format","example":"## Writing Style (learned from author edits)\\nTone: direct and conversational\\nStructure (email): Short paragraphs, subheadings"}]'::jsonb,
    false
  )
  RETURNING id, system_prompt, user_prompt, model, temperature, max_tokens
)
...
WITH inserted AS (
  ...
  VALUES (
    ...
    '',
    $usr$Role: You are a knowledge extraction specialist. Your job is to mine transcripts for business-valuable information and organize it into a structured knowledge base.
...
Return your response as valid JSON in this exact format:
{
  ...
  "total_count": number
    ...
    1.0,
    8000,
    '[{"name":"call_title_line","description":"Title line for the call (optional)","example":"Title: Weekly Coaching Call - Cold Email Mastery"},{"name":"participants_line","description":"Participants line (optional)","example":"Participants: Tim Smith, Jane Doe, Mike Johnson"},{"name":"call_date_line","description":"Date line for the call (optional)","example":"Date: 2026-02-24"},{"name":"transcript_type","description":"Type of transcript: coaching or sales","example":"coaching"},{"name":"speaker_context","description":"Speaker map context with roles and companies for proper attribution","example":"Speaker Context:\\n- \"Tim Smith\" is the HOST from Modern Agency Sales\\n- \"Jane Doe\" is a CLIENT from Acme Corp\\n\\nIMPORTANT: When extracting knowledge, attribute insights to the correct person and company."},{"name":"type_guidance","description":"Type-specific extraction guidance (coaching vs sales focus areas)","example":"This is a GROUP COACHING CALL where the host teaches. Focus heavily on:\\n- Insights: methods, frameworks, principles..."},{"name":"transcript","description":"The raw transcript text (truncated to 25,000 characters)","example":"Tim: Welcome everyone. Today we are going to talk about..."}]'::jsonb,
    false
  )
  RETURNING id, system_prompt, user_prompt, model, temperature, max_tokens
)
...
WITH inserted AS (
  ...
  VALUES (
    ...
    '',
    $usr$Given this topic and knowledge base context, suggest 3-5 unique angles for a LinkedIn post.
...
Example: ["Contrarian take on why X actually hurts more than it helps", "Step-by-step breakdown of the process that generated $Y"]$usr$,
    'claude-sonnet-4-6',
    1.0,
    500,
    '[{"name":"topic","description":"The topic to generate content angles for","example":"Cold email personalization"},{"name":"compiled_context","description":"Compiled knowledge base context grouped by knowledge type (truncated to 3000 chars)","example":"KEY INSIGHTS:\\n- Personalized first lines get 3x replies\\n- Most people over-personalize...\\n\\nSTORIES:\\n- Client switched to 1-sentence personalization..."},{"name":"voice_style_section","description":"Optional author style preferences section from buildVoicePromptSection()","example":"\\nAUTHOR STYLE PREFERENCES:\\n## Writing Style (learned from author edits)\\nTone: direct and conversational\\n\\nSuggest angles that align with this author''s voice and preferences.\\n"}]'::jsonb,
    false
  )
  RETURNING id, system_prompt, user_prompt, model, temperature, max_tokens
)
...
WITH inserted AS (
  ...
  VALUES (
    ...
    '',
    $usr$Analyze what changed between the original and edited text. Identify specific writing style patterns.
...
Return {"patterns": []} if no meaningful style changes detected.$usr$,
    'claude-haiku-4-5-20250514',
    1.0,
    500,
    '[{"name":"content_type","description":"Type of content being edited (e.g., linkedin_post, email, lead_magnet)","example":"linkedin_post"},{"name":"field_name","description":"Name of the field being edited","example":"content"},{"name":"original_text","description":"The original text before editing","example":"Here''s the thing about lead magnets. They''re a game-changer for your business."},{"name":"edited_text","description":"The edited text after changes","example":"I used to think lead magnets were just PDFs. Then one generated $47k in pipeline."}]'::jsonb,
    false
  )
  RETURNING id, system_prompt, user_prompt, model, temperature, max_tokens
)
...
WITH inserted AS (
  ...
  VALUES (
    ...
    '',
    $usr$Synthesize these knowledge entries about "{{topic_name}}" into a concise briefing (200-400 words).
...
KNOWLEDGE ENTRIES:
{{knowledge_sections}}$usr$,
    'claude-haiku-4-5-20250514',
    1.0,
    1500,
    '[{"name":"topic_name","description":"The topic being summarized","example":"Cold Email"},{"name":"knowledge_sections","description":"Formatted knowledge entries grouped by type, with quality-sorted entries as bullet points","example":"## Insight (5)\\n- Best cold emails are 3 sentences max\\n- Subject lines with numbers get 2x opens\\n\\n## Story (2)\\n- Client switched from long form to 3 sentences, reply rate went from 2% to 8%"}]'::jsonb,
    false
  )
  RETURNING id, system_prompt, user_prompt, model, temperature, max_tokens
)
...
WITH inserted AS (
  ...
  VALUES (
    ...
    '',
    $usr$You are a writing style analyst. Given a current voice profile and recent edit patterns, produce an updated voice profile.
...
Return ONLY the JSON object, no explanation.$usr$,
    'claude-sonnet-4-5-20250514',
    1.0,
    2000,
    '[{"name":"current_profile_json","description":"Current voice profile as JSON string","example":"{\\n  \"tone\": \"direct and conversational\",\\n  \"banned_phrases\": [\"game-changer\"],\\n  \"evolution_version\": 2\\n}"},{"name":"edit_count","description":"Total number of edits analyzed","example":"12"},{"name":"aggregated_patterns","description":"Aggregated edit patterns with counts and descriptions","example":"- shortened_hook (5x): Consistently shortens hooks to under 10 words\\n- removed_jargon (3x): Removes business jargon in favor of plain language"},{"name":"sample_edits","description":"Up to 5 most recent edits with original, edited text, CEO notes, and tags","example":"Original: \"Here''s the thing about lead magnets...\"\\nEdited: \"I spent $50k testing lead magnets...\"\\nCEO note: More specific, lead with results\\nTags: shortened_hook, added_specifics\\n---\\nOriginal: ..."}]'::jsonb,
    false
  )
  RETURNING id, system_prompt, user_prompt, model, temperature, max_tokens
)
...
WITH inserted AS (
  ...
  VALUES (
    ...
    '',
    $usr$## Writing Style (learned from author edits)
{{tone_line}}
...
{{positive_examples_count}} examples on file — match this quality.$usr$,
    'claude-sonnet-4-6',
    0,
    0,
    '[{"name":"tone_line","description":"Tone descriptor from profile","example":"Tone: direct and conversational"},{"name":"vocabulary_avoid_line","description":"Comma-separated list of words to avoid","example":"Vocabulary: AVOID: leverage, synergy, game-changer"},{"name":"vocabulary_prefer_line","description":"Comma-separated list of preferred words","example":"Vocabulary: PREFER: specific, concrete, actionable"},{"name":"banned_phrases_line","description":"Comma-separated list of banned phrases","example":"NEVER use these phrases: here''s the thing, at the end of the day"},{"name":"structure_line","description":"Structure patterns for the content type (linkedin or email)","example":"Structure (linkedin): Short hooks under 10 words. End with question CTA."},{"name":"cta_style_line","description":"Preferred CTA style","example":"CTA style: Question-based, not command-based"},{"name":"length_target_line","description":"Target content length for the content type","example":"Length target: 200-300 words"},{"name":"storytelling_line","description":"Preferred storytelling approach","example":"Storytelling: Lead with specific outcome, then backstory"},{"name":"edit_patterns_list","description":"Recent edit patterns with descriptions, counts, and confidence scores (filtered to confidence >= 30%)","example":"- Consistently shortens hooks to under 10 words (5x, confidence: 83%)\\n- Removes business jargon in favor of plain language (3x, confidence: 50%)"},{"name":"positive_examples_count","description":"Number of positive examples (posts marked \"Good as-is\") on file","example":"7"}]'::jsonb,
    false
  )
  RETURNING id, system_prompt, user_prompt, model, temperature, max_tokens
)
